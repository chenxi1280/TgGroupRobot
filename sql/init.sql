...
CREATE TABLE IF NOT EXISTS bot.chat_settings (
    ...
    verification_mode VARCHAR(16) NOT NULL DEFAULT 'button',       -- 验证模式（button/math/captcha/admin）
...
COMMENT ON COLUMN bot.chat_settings.verification_mode IS '验证模式：button（按钮验证）、math（数学题）、captcha（验证码）、admin（管理员确认）';
COMMENT ON COLUMN bot.chat_settings.verification_timeout_seconds IS '新人验证超时时间（秒），超时后自动处理（admin 模式不生效）';
