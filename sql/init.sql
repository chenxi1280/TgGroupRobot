...
COMMENT ON COLUMN chat_settings.verification_mode IS '验证模式：button（按钮验证）、math（数学题）、captcha（验证码）';
...
COMMENT ON COLUMN chat_settings.welcome_enabled IS '是否启用进群欢迎消息';
COMMENT ON COLUMN chat_settings.welcome_message IS '自定义欢迎消息模板，可使用变量如 {username}';
COMMENT ON COLUMN chat_settings.anti_flood_enabled IS '是否启用反刷屏检测';
COMMENT ON COLUMN chat_settings.anti_flood_messages IS '反刷屏触发消息数量';
COMMENT ON COLUMN chat_settings.anti_flood_seconds IS '反刷屏检测时间窗口（秒）';
COMMENT ON COLUMN chat_settings.anti_flood_action IS '反刷屏惩罚动作：mute（禁言）、delete（删除）、ban（封禁）';
COMMENT ON COLUMN chat_settings.anti_flood_mute_duration IS '反刷屏禁言时长（秒）';
...
COMMENT ON COLUMN verification_challenges.verification_type IS '验证类型：button（按钮验证）、math（数学题）、captcha（验证码）';
COMMENT ON COLUMN verification_challenges.question IS '验证问题，用于数学题模式等';
COMMENT ON COLUMN verification_challenges.answer IS '验证答案，用于验证用户输入';
...
CREATE TABLE IF NOT EXISTS conversation_states (
    ...
    CONSTRAINT fk_conversation_states_chat_id FOREIGN KEY (chat_id)
        REFERENCES tg_chats(id) ON DELETE CASCADE,                -- 外键约束：删除群组时级联删除状态
    CONSTRAINT fk_conversation_states_user_id FOREIGN KEY (user_id)
        REFERENCES tg_users(id) ON DELETE CASCADE,                -- 外键约束：删除用户时级联删除状态
    CONSTRAINT uq_conversation_state UNIQUE (chat_id, user_id)    -- 唯一约束：同一群组中同一用户只能有一个状态
);

COMMENT ON TABLE conversation_states IS '对话状态表，存储用户对话状态，用于多步骤交互';
COMMENT ON COLUMN conversation_states.id IS '自增主键';
COMMENT ON COLUMN conversation_states.chat_id IS '群组 ID，外键关联 tg_chats.id';
COMMENT ON COLUMN conversation_states.user_id IS '用户 ID，外键关联 tg_users.id';
COMMENT ON COLUMN conversation_states.state_type IS '状态类型，标识当前处于什么对话流程';
COMMENT ON COLUMN conversation_states.state_data IS '状态数据，JSONB 格式存储对话过程中的临时数据';
COMMENT ON COLUMN conversation_states.created_at IS '记录创建时间';
COMMENT ON COLUMN conversation_states.updated_at IS '记录最后更新时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_conversation_states_chat_id ON conversation_states(chat_id);
CREATE INDEX IF NOT EXISTS ix_conversation_states_user_id ON conversation_states(user_id);
CREATE INDEX IF NOT EXISTS ix_conversation_states_state_type ON conversation_states(state_type);
...
CREATE TABLE IF NOT EXISTS lotteries (
    ...
    CONSTRAINT fk_lotteries_chat_id FOREIGN KEY (chat_id)
        REFERENCES tg_chats(id) ON DELETE CASCADE,                -- 外键约束：删除群组时级联删除抽奖
    CONSTRAINT fk_lotteries_created_by_user_id FOREIGN KEY (created_by_user_id)
        REFERENCES tg_users(id) ON DELETE SET NULL                -- 外键约束：删除用户时将创建者设为 NULL
);

COMMENT ON TABLE lotteries IS '抽奖表，存储群组内的抽奖活动信息';
COMMENT ON COLUMN lotteries.id IS '自增主键';
COMMENT ON COLUMN lotteries.chat_id IS '群组 ID，外键关联 tg_chats.id';
COMMENT ON COLUMN lotteries.created_by_user_id IS '创建抽奖的用户 ID，外键关联 tg_users.id，删除用户时设为 NULL';
COMMENT ON COLUMN lotteries.title IS '抽奖标题，默认为"通用抽奖"';
COMMENT ON COLUMN lotteries.description IS '抽奖描述说明，可包含活动详情、规则等';
COMMENT ON COLUMN lotteries.draw_time IS '计划开奖时间';
COMMENT ON COLUMN lotteries.prizes IS '奖品列表，JSONB 数组格式存储，如：[{"name": "一等奖", "quantity": 1}]';
COMMENT ON COLUMN lotteries.draw_mode IS '开奖模式：random（随机开奖）、manual（手动指定中奖人）';
COMMENT ON COLUMN lotteries.status IS '抽奖状态：pending（待开奖）、completed（已完成）、cancelled（已取消）';
COMMENT ON COLUMN lotteries.message_id IS '抽奖消息的 Telegram message_id，用于更新消息';
COMMENT ON COLUMN lotteries.min_points IS '参与抽奖的最低积分要求，0表示无限制';
COMMENT ON COLUMN lotteries.max_participants IS '最大参与人数，0表示无限制';
COMMENT ON COLUMN lotteries.participation_cost IS '参与抽奖需要消耗的积分，0表示免费参与';
COMMENT ON COLUMN lotteries.join_start_time IS '参与开始时间，NULL表示创建后立即可参与';
COMMENT ON COLUMN lotteries.join_end_time IS '参与结束时间，NULL表示到开奖时间前都可参与';
COMMENT ON COLUMN lotteries.requirement_days IS '入群天数要求，0表示无限制';
COMMENT ON COLUMN lotteries.created_at IS '抽奖创建时间';
COMMENT ON COLUMN lotteries.drawn_at IS '实际开奖时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_lotteries_chat_id ON lotteries(chat_id);
CREATE INDEX IF NOT EXISTS ix_lotteries_draw_time ON lotteries(draw_time);
...
CREATE TABLE IF NOT EXISTS lottery_participants (
    ...
    CONSTRAINT fk_lottery_participants_lottery_id FOREIGN KEY (lottery_id)
        REFERENCES lotteries(id) ON DELETE CASCADE,               -- 外键约束：删除抽奖时级联删除参与者
    CONSTRAINT fk_lottery_participants_user_id FOREIGN KEY (user_id)
        REFERENCES tg_users(id) ON DELETE CASCADE,                -- 外键约束：删除用户时级联删除参与记录
    CONSTRAINT uq_lottery_participant UNIQUE (lottery_id, user_id) -- 唯一约束：同一用户在同一抽奖中只能参与一次
);

COMMENT ON TABLE lottery_participants IS '抽奖参与者表，存储参与抽奖的用户信息';
COMMENT ON COLUMN lottery_participants.id IS '自增主键';
COMMENT ON COLUMN lottery_participants.lottery_id IS '抽奖 ID，外键关联 lotteries.id';
COMMENT ON COLUMN lottery_participants.user_id IS '用户 ID，外键关联 tg_users.id';
COMMENT ON COLUMN lottery_participants.points_balance IS '参与时的积分余额快照，用于审计';
COMMENT ON COLUMN lottery_participants.created_at IS '用户参与抽奖的时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_lottery_participants_lottery_id ON lottery_participants(lottery_id);
CREATE INDEX IF NOT EXISTS ix_lottery_participants_user_id ON lottery_participants(user_id);
...
CREATE TABLE IF NOT EXISTS lottery_winners (
    ...
    CONSTRAINT fk_lottery_winners_lottery_id FOREIGN KEY (lottery_id)
        REFERENCES lotteries(id) ON DELETE CASCADE,               -- 外键约束：删除抽奖时级联删除中奖记录
    CONSTRAINT fk_lottery_winners_user_id FOREIGN KEY (user_id)
        REFERENCES tg_users(id) ON DELETE CASCADE                 -- 外键约束：删除用户时级联删除中奖记录
);

COMMENT ON TABLE lottery_winners IS '抽奖中奖记录表，存储抽奖的中奖结果';
COMMENT ON COLUMN lottery_winners.id IS '自增主键';
COMMENT ON COLUMN lottery_winners.lottery_id IS '抽奖 ID，外键关联 lotteries.id';
COMMENT ON COLUMN lottery_winners.user_id IS '中奖用户 ID，外键关联 tg_users.id';
COMMENT ON COLUMN lottery_winners.prize_name IS '中奖奖品名称，如"1USDT"';
COMMENT ON COLUMN lottery_winners.prize_index IS '奖品索引，对应 lotteries.prizes 数组中的位置，从0开始';
COMMENT ON COLUMN lottery_winners.created_at IS '中奖时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_lottery_winners_lottery_id ON lottery_winners(lottery_id);
CREATE INDEX IF NOT EXISTS ix_lottery_winners_user_id ON lottery_winners(user_id);
...
CREATE TABLE IF NOT EXISTS scheduled_messages (
    ...
    CONSTRAINT fk_scheduled_messages_chat_id FOREIGN KEY (chat_id)
        REFERENCES tg_chats(id) ON DELETE CASCADE,               -- 外键约束：删除群组时级联删除定时消息
    CONSTRAINT fk_scheduled_messages_created_by_user_id FOREIGN KEY (created_by_user_id)
        REFERENCES tg_users(id) ON DELETE SET NULL               -- 外键约束：删除用户时将创建者设为 NULL
);

COMMENT ON TABLE scheduled_messages IS '定时消息表，存储定时发送的消息';
COMMENT ON COLUMN scheduled_messages.id IS '自增主键';
COMMENT ON COLUMN scheduled_messages.chat_id IS '群组 ID，外键关联 tg_chats.id';
COMMENT ON COLUMN scheduled_messages.created_by_user_id IS '创建消息的用户 ID，外键关联 tg_users.id，删除用户时设为 NULL';
COMMENT ON COLUMN scheduled_messages.content IS '消息内容';
COMMENT ON COLUMN scheduled_messages.schedule_type IS '定时类型：none（一次性）、daily（每天）、weekly（每周）、custom（自定义间隔）';
COMMENT ON COLUMN scheduled_messages.interval_minutes IS '自定义间隔分钟数';
COMMENT ON COLUMN scheduled_messages.is_active IS '是否激活（true=启用，false=禁用）';
COMMENT ON COLUMN scheduled_messages.next_send_time IS '下次发送时间';
COMMENT ON COLUMN scheduled_messages.last_sent_at IS '上次发送时间';
COMMENT ON COLUMN scheduled_messages.send_count IS '已发送次数统计';
COMMENT ON COLUMN scheduled_messages.created_at IS '消息创建时间';
COMMENT ON COLUMN scheduled_messages.updated_at IS '记录最后更新时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_scheduled_messages_chat_id ON scheduled_messages(chat_id);
CREATE INDEX IF NOT EXISTS ix_scheduled_messages_schedule_type ON scheduled_messages(schedule_type);
CREATE INDEX IF NOT EXISTS ix_scheduled_messages_is_active ON scheduled_messages(is_active);
CREATE INDEX IF NOT EXISTS ix_scheduled_messages_next_send_time ON scheduled_messages(next_send_time);
...
CREATE TABLE IF NOT EXISTS auto_reply_rules (
    ...
    CONSTRAINT fk_auto_reply_rules_chat_id FOREIGN KEY (chat_id)
        REFERENCES tg_chats(id) ON DELETE CASCADE,               -- 外键约束：删除群组时级联删除规则
    CONSTRAINT fk_auto_reply_rules_created_by_user_id FOREIGN KEY (created_by_user_id)
        REFERENCES tg_users(id) ON DELETE SET NULL               -- 外键约束：删除用户时将创建者设为 NULL
);

COMMENT ON TABLE auto_reply_rules IS '自动回复规则表，存储群组的自动回复规则';
COMMENT ON COLUMN auto_reply_rules.id IS '自增主键';
COMMENT ON COLUMN auto_reply_rules.chat_id IS '群组 ID，外键关联 tg_chats.id';
COMMENT ON COLUMN auto_reply_rules.created_by_user_id IS '创建规则的用户 ID，外键关联 tg_users.id，删除用户时设为 NULL';
COMMENT ON COLUMN auto_reply_rules.keywords IS '触发关键词列表，JSONB 数组格式存储';
COMMENT ON COLUMN auto_reply_rules.reply_content IS '自动回复的内容';
COMMENT ON COLUMN auto_reply_rules.match_type IS '匹配类型：contains（包含匹配）、exact（精确匹配）、regex（正则表达式）';
COMMENT ON COLUMN auto_reply_rules.is_active IS '是否激活（true=启用，false=禁用）';
COMMENT ON COLUMN auto_reply_rules.match_count IS '规则被触发的次数统计';
COMMENT ON COLUMN auto_reply_rules.case_sensitive IS '是否区分大小写';
COMMENT ON COLUMN auto_reply_rules.created_at IS '规则创建时间';
COMMENT ON COLUMN auto_reply_rules.updated_at IS '记录最后更新时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_auto_reply_rules_chat_id ON auto_reply_rules(chat_id);
CREATE INDEX IF NOT EXISTS ix_auto_reply_rules_is_active ON auto_reply_rules(is_active);
...
CREATE TABLE IF NOT EXISTS banned_words (
    ...
    CONSTRAINT fk_banned_words_chat_id FOREIGN KEY (chat_id)
        REFERENCES tg_chats(id) ON DELETE CASCADE,               -- 外键约束：删除群组时级联删除违禁词
    CONSTRAINT fk_banned_words_created_by_user_id FOREIGN KEY (created_by_user_id)
        REFERENCES tg_users(id) ON DELETE SET NULL               -- 外键约束：删除用户时将创建者设为 NULL
);

COMMENT ON TABLE banned_words IS '违禁词表，存储群组的违禁词规则';
COMMENT ON COLUMN banned_words.id IS '自增主键';
COMMENT ON COLUMN banned_words.chat_id IS '群组 ID，外键关联 tg_chats.id';
COMMENT ON COLUMN banned_words.created_by_user_id IS '创建规则的用户 ID，外键关联 tg_users.id，删除用户时设为 NULL';
COMMENT ON COLUMN banned_words.word IS '违禁词内容';
COMMENT ON COLUMN banned_words.match_type IS '匹配类型：contains（包含匹配）、exact（精确匹配）、regex（正则表达式）';
COMMENT ON COLUMN banned_words.action IS '惩罚动作：delete（删除消息）、mute（禁言）、ban（封禁用户）';
COMMENT ON COLUMN banned_words.mute_duration IS '禁言时长（秒）';
COMMENT ON COLUMN banned_words.notify IS '是否发送删除提醒给用户';
COMMENT ON COLUMN banned_words.notify_message IS '自定义提醒消息';
COMMENT ON COLUMN banned_words.is_active IS '是否激活（true=启用，false=禁用）';
COMMENT ON COLUMN banned_words.trigger_count IS '违禁词被触发的次数统计';
COMMENT ON COLUMN banned_words.case_sensitive IS '是否区分大小写';
COMMENT ON COLUMN banned_words.created_at IS '规则创建时间';
COMMENT ON COLUMN banned_words.updated_at IS '记录最后更新时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_banned_words_chat_id ON banned_words(chat_id);
CREATE INDEX IF NOT EXISTS ix_banned_words_word ON banned_words(word);
CREATE INDEX IF NOT EXISTS ix_banned_words_is_active ON banned_words(is_active);
...
CREATE TABLE IF NOT EXISTS invite_links (
    ...
    CONSTRAINT fk_invite_links_chat_id FOREIGN KEY (chat_id)
        REFERENCES tg_chats(id) ON DELETE CASCADE,               -- 外键约束：删除群组时级联删除邀请链接
    CONSTRAINT fk_invite_links_created_by_user_id FOREIGN KEY (created_by_user_id)
        REFERENCES tg_users(id) ON DELETE SET NULL               -- 外键约束：删除用户时将创建者设为 NULL
);

COMMENT ON TABLE invite_links IS '邀请链接管理表，存储群组邀请链接管理信息';
COMMENT ON COLUMN invite_links.id IS '自增主键';
COMMENT ON COLUMN invite_links.chat_id IS '群组 ID，外键关联 tg_chats.id';
COMMENT ON COLUMN invite_links.created_by_user_id IS '创建链接的用户 ID，外键关联 tg_users.id，删除用户时设为 NULL';
COMMENT ON COLUMN invite_links.invite_link IS 'Telegram邀请链接';
COMMENT ON COLUMN invite_links.name IS '链接名称，便于识别管理';
COMMENT ON COLUMN invite_links.status IS '链接状态：active（有效）、expired（已过期）、revoked（已撤销）';
COMMENT ON COLUMN invite_links.member_limit IS '成员数量限制，NULL表示无限制';
COMMENT ON COLUMN invite_links.member_count IS '当前通过此链接加入的成员数';
COMMENT ON COLUMN invite_links.expire_date IS '链接过期时间，NULL表示永久有效';
COMMENT ON COLUMN invite_links.creates_join_request IS '是否需要审核（true=需要管理员审核）';
COMMENT ON COLUMN invite_links.created_at IS '链接创建时间';
COMMENT ON COLUMN invite_links.updated_at IS '记录最后更新时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_invite_links_chat_id ON invite_links(chat_id);
CREATE INDEX IF NOT EXISTS ix_invite_links_invite_link ON invite_links(invite_link);
CREATE INDEX IF NOT EXISTS ix_invite_links_status ON invite_links(status);

-- ============================================
-- 21. 接龙活动表 (solitaires)
-- 存储群组接龙活动信息
-- ============================================
CREATE TABLE IF NOT EXISTS solitaires (
    id SERIAL PRIMARY KEY,                                         -- 自增主键
    chat_id BIGINT NOT NULL,                                      -- 群组 ID（外键关联 tg_chats.id）
    created_by_user_id BIGINT,                                   -- 创建者用户 ID（外键关联 tg_users.id，可为空）
    ...
    CONSTRAINT fk_solitaires_chat_id FOREIGN KEY (chat_id)
        REFERENCES tg_chats(id) ON DELETE CASCADE,               -- 外键约束：删除群组时级联删除接龙
    CONSTRAINT fk_solitaires_created_by_user_id FOREIGN KEY (created_by_user_id)
        REFERENCES tg_users(id) ON DELETE SET NULL               -- 外键约束：删除用户时将创建者设为 NULL
);

COMMENT ON TABLE solitaires IS '接龙活动表，存储群组接龙活动信息';
COMMENT ON COLUMN solitaires.id IS '自增主键';
COMMENT ON COLUMN solitaires.chat_id IS '群组 ID，外键关联 tg_chats.id';
COMMENT ON COLUMN solitaires.created_by_user_id IS '创建接龙的用户 ID，外键关联 tg_users.id，删除用户时设为 NULL';
COMMENT ON COLUMN solitaires.title IS '接龙标题';
COMMENT ON COLUMN solitaires.description IS '接龙描述说明';
COMMENT ON COLUMN solitaires.status IS '接龙状态：active（进行中）、closed（已结束）';
COMMENT ON COLUMN solitaires.max_participants IS '最大参与人数，NULL表示无限制';
COMMENT ON COLUMN solitaires.entries IS '参与记录，JSONB 数组格式存储，如：[{"user_id": 123, "username": "xxx", "content": "xxx", "joined_at": "2024-01-01"}]';
COMMENT ON COLUMN solitaires.message_id IS '接龙消息的 Telegram message_id，用于更新消息';
COMMENT ON COLUMN solitaires.created_at IS '接龙创建时间';
COMMENT ON COLUMN solitaires.updated_at IS '记录最后更新时间';

-- 创建索引以优化查询性能
CREATE INDEX IF NOT EXISTS ix_solitaires_chat_id ON solitaires(chat_id);
CREATE INDEX IF NOT EXISTS ix_solitaires_status ON solitaires(status);
