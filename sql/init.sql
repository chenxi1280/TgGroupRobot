...
CREATE TABLE IF NOT EXISTS bot.chat_settings (
    ...
    night_mode_enabled BOOLEAN NOT NULL DEFAULT FALSE,            -- 夜间管控总开关
    night_mode_start_time VARCHAR(5),                             -- 夜间管控开始时间（HH:MM）
    night_mode_end_time VARCHAR(5),                               -- 夜间管控结束时间（HH:MM）
    night_mode_exempt_admin BOOLEAN NOT NULL DEFAULT TRUE,        -- 夜间管控是否豁免管理员
    night_mode_whitelist_user_ids JSONB NOT NULL DEFAULT '[]'::jsonb, -- 夜间管控白名单用户ID
    night_mode_delete_message BOOLEAN NOT NULL DEFAULT TRUE,      -- 夜间管控删除消息
    night_mode_warn_enabled BOOLEAN NOT NULL DEFAULT TRUE,        -- 夜间管控提示开关
    night_mode_warn_text TEXT NOT NULL DEFAULT '🌙 夜间管控生效中，请稍后再试。', -- 夜间管控提示文案
    night_mode_warn_delete_after_seconds INTEGER NOT NULL DEFAULT 60, -- 夜间管控提示删除秒数
...
COMMENT ON COLUMN bot.chat_settings.night_mode_enabled IS '夜间管控总开关';
COMMENT ON COLUMN bot.chat_settings.night_mode_start_time IS '夜间管控开始时间（HH:MM）';
COMMENT ON COLUMN bot.chat_settings.night_mode_end_time IS '夜间管控结束时间（HH:MM）';
COMMENT ON COLUMN bot.chat_settings.night_mode_exempt_admin IS '夜间管控是否豁免管理员';
COMMENT ON COLUMN bot.chat_settings.night_mode_whitelist_user_ids IS '夜间管控白名单用户ID';
COMMENT ON COLUMN bot.chat_settings.night_mode_delete_message IS '夜间管控删除消息';
COMMENT ON COLUMN bot.chat_settings.night_mode_warn_enabled IS '夜间管控提示开关';
COMMENT ON COLUMN bot.chat_settings.night_mode_warn_text IS '夜间管控提示文案';
COMMENT ON COLUMN bot.chat_settings.night_mode_warn_delete_after_seconds IS '夜间管控提示删除秒数';
...
CREATE TABLE IF NOT EXISTS bot.scheduled_message_logs (
    ...
    chat_id BIGINT NOT NULL,                                        -- 群组 ID
    run_key VARCHAR(128) NOT NULL,                                  -- task_id + scheduled_for 唯一执行键
    scheduled_for BIGINT NOT NULL,                                  -- 计划发送时间（Unix 时间戳）
    content_snapshot JSONB NOT NULL,                                -- occurrence 创建时的完整发送快照
    status VARCHAR(32) NOT NULL DEFAULT 'pending',                  -- 执行状态
    attempt_count INTEGER NOT NULL DEFAULT 0,                       -- 已认领尝试次数
    next_retry_at TIMESTAMPTZ,                                     -- 下次自动重试时间
    lease_until TIMESTAMPTZ,                                       -- processing 租约截止时间
    send_started_at TIMESTAMPTZ,                                   -- Telegram 调用开始时间
    completed_at TIMESTAMPTZ,                                      -- 终态完成时间
    error_code VARCHAR(64),                                        -- 机器可读错误码
    message_id INTEGER,                                             -- 消息 ID（Telegram message_id）
    sent_at TIMESTAMPTZ,                                            -- 成功发送时间（兼容字段）
    success BOOLEAN,                                                -- 终态结果（兼容字段）
...
CREATE TABLE IF NOT EXISTS bot.auction_settings (
    ...
    create_permission VARCHAR(16) NOT NULL DEFAULT 'all',
