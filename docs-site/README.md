...
这是当前仓库内独立维护的轻量静态文档站，用于展示 TgGroupRobot 的功能步骤、Telegram 风格操作图和常见问题。
...
- 功能详情页由 `src/pages/features/[slug].astro` 动态生成，不再为每个功能维护单独 MDX 页面。
...
- 不引入 Starlight、MDX、Mermaid 和 Pagefind，避免部署镜像携带重型文档站依赖。
