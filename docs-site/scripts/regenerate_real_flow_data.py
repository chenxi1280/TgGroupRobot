...
def build_flows() -> dict[str, dict[str, Any]]:
    ...
    flows["bottom-button"] = admin_menu_button_flow(
        ...
        [
            ...
            [button("立即生成", f"btm:generate:{CHAT}:now", "goto", "result", "已生成底部按钮消息。"), button("重复发送", f"btm:repeat:{CHAT}:1", "toggle", "result", "重复发送已开启。")],
        ],
        ["bottom_button_text_input", "bottom_button_button_text_input", "bottom_button_payload_input"],
        ["backend/features/admin/activity/bottom_button.py", "backend/features/admin/activity/bottom_button_input.py", "backend/features/group_ops/bottom_button_router.py"],
        [
            screen("layout", "按钮布局", ["添加按钮后进入按钮详情。"], [[button("添加按钮", f"btm:layout:{CHAT}:add:1:1", "goto", "button_detail", "已添加按钮。")], [button("返回底部按钮", f"btm:home:{CHAT}", "back", "home", "已返回底部按钮。")]]),
            screen("button_detail", "按钮详情", ["按钮可设置文本、payload 和点击模式。"], [[button("按钮文本", f"btm:button:{CHAT}:text:1", "input", "button_text_input", "请输入按钮文本。"), button("Payload", f"btm:button:{CHAT}:payload:1", "input", "payload_input", "请输入 payload。")], [button("发送模式", f"btm:button:{CHAT}:mode:1:send", "toggle", "result", "已设置发送模式。"), button("填充模式", f"btm:button:{CHAT}:mode:1:fill", "toggle", "result", "已设置填充模式。"), button("删除按钮", f"btm:button:{CHAT}:delete:1", "confirm", "result", "按钮已删除。")], [button("运行时点击", f"btmrun:send:{CHAT}:1", "goto", "result", "用户点击后 Bot 会按 payload 发送。")]]),
            ...
            input_screen("payload_input", "设置按钮 Payload", "发送点击后发送的文本", "我要联系客服", f"btm:button:{CHAT}:detail:1", "button_detail", examples=[("Payload", "我要联系客服", "result")]),
        ],
    )
