...
async def _handle_review_setting_input(
    update, context, session, *, service, state_type: str,
    target_chat_id: int, text_value: str, message_text: str,
) -> bool:
    if state_type == "car_review_reward_points_input":
        if not re.fullmatch(r"\d+", text_value):
            ...
            return True
        await _save_review_setting_input(
            update, context, session, service=service,
            target_chat_id=target_chat_id,
            changes={"reward_points": int(text_value)},
        )
        return True
    if state_type in {"car_review_submit_command_input", "car_review_rank_command_input"}:
        if not text_value:
            ...
            return True
        field = "submit_command" if state_type == "car_review_submit_command_input" else "rank_command"
        await _save_review_setting_input(
            update, context, session, service=service,
            target_chat_id=target_chat_id, changes={field: text_value[:64]},
        )
        return True
    if state_type == "car_review_template_input":
        ...
        await _save_review_setting_input(
            update, context, session, service=service,
            target_chat_id=target_chat_id, changes={"template_text": message_text},
        )
        return True
    return False


async def _save_review_setting_input(
    update, context, session, *, service, target_chat_id: int, changes: dict
) -> None:
    await service.update_setting(session, target_chat_id, **changes)
    await _finish_review_input(
        update, context, session, target_chat_id=target_chat_id
    )


async def _handle_review_approver_input(
    update, context, session, *, service, target_chat_id: int, text_value: str
) -> None:
    approver_id = None
    if text_value != "清空":
        try:
            user = await service.resolve_approver(session, text_value)
        except ValidationError as exc:
            await update.effective_message.reply_text(str(exc))
            return
        approver_id = user.id
    await service.update_setting(
        session, target_chat_id, approver_user_id=approver_id
    )
    await _finish_review_input(
        update, context, session, target_chat_id=target_chat_id
    )
...
async def _handle_review_custom_field_input(
    update, context, session, *, service, state_type: str,
    state_data: dict, target_chat_id: int, text_value: str,
) -> bool:
    if state_type == "car_review_field_add_input":
        ...
        try:
            await service.add_custom_field(
                session, target_chat_id, field_key=parts[0], field_label=parts[1]
            )
        except ValidationError as exc:
            await update.effective_message.reply_text(str(exc))
            return True
    elif state_type == "car_review_field_label_input":
        ...
        if not isinstance(field_id, int):
            ...
            return True
        try:
            item = await service.update_custom_field_label(
                session, target_chat_id, field_id, field_label=text_value
            )
        except ValidationError as exc:
            await update.effective_message.reply_text(str(exc))
            return True
        ...
        return False
    await _finish_review_input(
        update, context, session, target_chat_id=target_chat_id, show_fields=True
    )
    return True
...
async def handle_car_review_feature_input(
    ...
    *, state,
    ...
    if await _handle_review_setting_input(
        update, context, session, service=CarReviewService,
        state_type=state_type, target_chat_id=target_chat_id,
        text_value=text_value, message_text=message_text,
    ):
        return True
    if state_type == "car_review_approver_input":
        await _handle_review_approver_input(
            update, context, session, service=CarReviewService,
            target_chat_id=target_chat_id, text_value=text_value,
        )
        return True
    return await _handle_review_custom_field_input(
        update, context, session, service=CarReviewService,
