        ...
        try:
            if ad.image_file_id:
                await publish_service.send_photo(
                    ...
                    caption=format_ad_push_text_func(ad),
                )
            else:
                await publish_service.send(
                    context,
                    chat_id=ad.chat_id,
            ...
            ad_updated = await get_ad_func(session, ad_id)
            text = format_ad_detail_text_func(ad_updated)
            await q.edit_message_text(text, reply_markup=ads_detail_keyboard_func(ad_id, ad_updated.enabled))
