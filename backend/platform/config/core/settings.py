...
def _is_missing_bot_token(error: ValidationError) -> bool:
    return any(
        tuple(item.get("loc", ())) == ("bot_token",) and item.get("type") == "missing"
        for item in error.errors()
    )


def _missing_bot_token_message(env_file: Path | None) -> str:
    project_root = Path(__file__).parent.parents[3]
    example_file = project_root / "config" / "env.example"
    location = (
        f"环境配置文件存在：{env_file}\n但缺少 BOT_TOKEN 配置。\n请在文件中添加：BOT_TOKEN=你的机器人令牌\n\n"
        if env_file is not None
        else (
            "未找到环境配置文件（.env 或 env）。\n"
            f"请参考示例文件创建：{example_file}\n\n"
            "创建步骤：\n1. 复制示例文件：cp config/env.example .env\n"
            "   或者：cp config/env.example env\n2. 编辑文件，设置你的 BOT_TOKEN\n"
            "   例如：BOT_TOKEN=1234567890:ABCdefGHIjklMNOpqrsTUVwxyz\n\n"
        )
    )
    return (
        "\n" + "=" * 60 + "\n配置错误：缺少必需的 BOT_TOKEN\n" + "=" * 60 + "\n\n"
        + location
        + "如何获取 BOT_TOKEN：\n1. 在 Telegram 中搜索 @BotFather\n"
        + "2. 发送 /newbot 命令创建新机器人\n"
        + "3. 按照提示完成创建后，BotFather 会提供 BOT_TOKEN\n"
        + "=" * 60 + "\n"
    )
...
def get_settings() -> Settings:
    ...
    try:
        ...
        if _is_missing_bot_token(e):
            raise ValueError(_missing_bot_token_message(env_file)) from e
        raise
