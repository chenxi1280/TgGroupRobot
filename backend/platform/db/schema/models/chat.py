...
class ChatSettings(Base):
    ...
    verification_cover_media_type: Mapped[str | None] = mapped_column(String(16), nullable=True)
    verification_cover_file_id: Mapped[str | None] = mapped_column(String(256), nullable=True)
    verification_agreement_text: Mapped[str] = mapped_column(Text, default="请阅读并同意本群规则后再发言。")
    verification_math_prompt_text: Mapped[str] = mapped_column(Text, default="请回答下面的简单算术题完成验证。")
    verification_wrong_action: Mapped[str] = mapped_column(String(16), default="none")
    verification_direct_mute_duration: Mapped[int] = mapped_column(Integer, default=0)
